...
data = pd.read_csv("/Users/jonathansong/Research_Folder/Panther/add_location_data.tsv", sep="\t")
...
y=["Validation_outcome"]
X=[i for i in data_final_vars if i not in y]

logreg = LogisticRegression()

X=data_final[X]
y=data_final["Validation_outcome"]
             
#X = sm.add_constant(X)
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=None)

logit_model=sm.Logit(y_train.astype(float), X_train.astype(float)).fit()
